...
upgrade.updateCard = function (req, res, next) {
  ...
  getCustomerByUser(req.session.user).then(function (results) {
    ...
    return req.app.stripe.customers.update(user.stripe_id, {
...
upgrade.invoice = function (req, res) {
  getCustomerByUser(req.session.user).then(function (results) {
    ...
    return req.app.stripe.invoices.retrieve(invoice_id).then(function (invoice) { // jshint ignore:line
...
upgrade.cancel = function (req, res, next) {
  return getCustomerByUser(req.session.user).then(function (results) {
    ...
    return req.app.stripe.customers.cancelSubscription(result.stripe_id, req.body.subscription, { at_period_end: true });// jshint ignore:line
  }).then(function () {
...
upgrade.features = function (req, res, next) {
  ...
  var app = req.app;
  var stripeProMonthURL = undefsafe(app.get("config"), 'payment.stripe.urls.month');
...
upgrade.payment = function (req, res, next) {
  ...
  if (req.query.coupon && req.query.coupon !== 'true') {
    ...
    promise = new Promise(function (resolve) {
      app.stripe.coupons.retrieve(req.query.coupon, function(error, coupon) {
  ...
  var testUnlimited = app.features('testUnlimited', req);
