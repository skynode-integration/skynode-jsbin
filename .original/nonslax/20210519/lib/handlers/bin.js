...
var async      = require('asyncjs'),
    ...
    metrics    = require("skynode-server").metrics,
...
module.exports = Observable.extend({
      ...
      new Promise(function (resolve, reject) {
        ...
        if (req.subdomain) {
          ...
          if (custom[req.subdomain]) {
            ...
            if (req.app.features('vanity', { session: { user: bin.metadata }})) {
              resolve();
            } else {
      ...
      this.validateBin(bin, function (err) {
        ...
        handler.models.bin.updatePanel(panel, params, function (err, result) {
          ...
          if (req.session.user) {
            ...
            if (req.app.features('dropbox', req) && !!dropbox_token) {
  ...
  protectVisibility: function (user, bin, cb) {
    console.log("debug:protectVisibility():" + bin);
  ...
  sendStart: function (req, res) {
    this.protectVisibility(req.session.user, req.bin, function (err, bin) {
      ...
      var template = this.templateFromBin(bin),
          ...
          sslForAll = req.app.features('sslForAll', req),
          ssl = req.secure,
      ...
      if (req.embed) {
        if (req.app.features('customEmbed', { session: { user: bin.metadata }})) {
  ...
  render: function renderEdit(req, res, bin) {
    ...
    res.render('index', {
      ...
      url: url,
      user: user || null,
      vanity: req.app.features('vanity', req) ? 'http://' + req.session.user.name + '.' + req.app.get('url host') : root,
    ...
    this.loadFiles(this.defaultFiles(), function (err, results) {
      ...
      if (!err) {
        if (next) {
          ...
          console.log("debug:result:" + results && results.url);
          req.bin = results;
          return next();
        } else {
          // depreciated
          console.log("debug:result2:");
          console.log("debug:result2:" + results && results.url);
  ...
  renderHistory: function (req, res, next, bins) {
    ...
    var acceptsJSON = req.header('Accept', '').indexOf('application/json') > -1,
        ...
        jsbin = JSON.stringify({ version: helpers.production ? helpers.set('version') : 'debug',
          root: helpers.url('', false), //---- true
          static: helpers.urlForStatic('', false) //---- true
        });
    ...
    this.formatHistory(bins, function (err, history) {
      if (acceptsJSON) {
        ...
        helpers.render('history', {
          ...
          function (err, html) {
    console.log("DEBUG:renderHistory:req.ajax:" + req.ajax);
            //req.ajax = true;
            if (req.ajax) {
              res.send(html);
            } else {
  ...
  loadFiles: function (files, fn) {
    ...
    async.files(files, path.join(__dirname,"../data") ).readFile('utf8').toArray(function (err, results) {
      cache.error = err;
      if (!err) {
        cache.result = {
          html: results[0].data,
  ...
  formatHistory: function (bins, format, fn) {
    ...
    if (fn === undefined) {
      ...
      format = function (bin, map) {
        ...
        console.log("debug:historybin:"+JSON.stringify(bin));
        console.log("debug:historybin:url"+helpers.urlForBin(bin));
        console.log("debug:historybin:editurl"+helpers.editUrlForBin(bin));
  ...
  delete: function (req, res, next) {
    ...
    if (req.app.features('admin', req) === false) {
