...
module.exports = Observable.extend({
    ...
    loadVanityURL: function(req, res, next) {
        if (req.subdomain) {
            ...
            userModel.load(req.subdomain, function(error, user) {
                ...
                if (req.app.features('vanity', { session: { user: user } })) {
                    // do magic
    ...
    validateRegister: function(req, res, next) {
        ...
        var flagged = req.body.flagged || false;
        var missingValues = [];
        var user = {
            name: username,
            email: email,
    ...
    validateLogin: function(req, res, next) {
        ...
        return userModel.load(username, function(err, user) {
            ...
            if (user.created && !!user.created.getTime) {
                return userModel.valid(key, user.key, validationComplete);
            } else {
                // No created timestamp so this is an old password. Validate it then
                // update it to use the new algorithm.
                return userModel.validOldKey(key, user.key, function(err, valid) {
                    ...
                    return userModel.upgradeKey(user.name, key, validationComplete);
                });
